...
const createAppoinment = catchAsync(async (req: Request & { user?: IAuthUser }, res: Response) => {
    const user = req.user
    const result = await AppoinmentServices.createAppointment(user as IAuthUser, req.body);
...
const updateAppoinmentStatus = catchAsync(async (req: Request & { user?: IAuthUser }, res: Response) => {

    const { id } = req.params;
    const { status } = req.body
    const user = req.body

    const result = await AppoinmentServices.updateAppoinmentStatus(id, status, user as IAuthUser);

    sendResponse(res, {
        statusCode: httpStatus.OK,
        success: true,
        message: "Appoinment status  successfully",
        data: result,
    });
});
